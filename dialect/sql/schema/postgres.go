...
func (d *Postgres) table(ctx context.Context, tx dialect.Tx, name string) (*Table, error) {
	...
	query, args := sql.Dialect(dialect.Postgres).
		Select(
			...
			"numeric_precision", "numeric_scale", "character_maximum_length", "generation_expression",
		).
...
func (d *Postgres) scanColumn(c *Column, rows *sql.Rows) error {
	...
	if err := rows.Scan(&c.Name, &c.typ, &nullable, &defaults, &udt, &numericPrecision, &numericScale, &characterMaximumLen, &generatedExpression); err != nil {
	...
	case "cidr", "inet", "macaddr", "macaddr8", "tsvector":
	...
	if generatedExpression.Valid {
		c.GeneratedExprs = map[string]GeneratedExpr{dialect.Postgres: {Expr: generatedExpression.String, Type: "STORED"}}
	}
	switch {
...
func (d *Postgres) addColumn(c *Column) *sql.ColumnBuilder {
	...
	if c.GeneratedExprs != nil && c.GeneratedExprs[dialect.Postgres].Expr != "" {
		b.Attr(fmt.Sprintf("GENERATED ALWAYS AS (%s) %s", c.GeneratedExprs[dialect.Postgres].Expr, c.GeneratedExprs[dialect.Postgres].Type))
	} else {
		c.nullable(b)
	}
