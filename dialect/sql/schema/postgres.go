...
func (d *Postgres) scanColumn(c *Column, rows *sql.Rows) error {
	...
	if generatedExpression.Valid {
		c.Generated = map[string]GeneratedExpr{dialect.Postgres: {Expr: generatedExpression.String, Type: "STORED"}}
	}
	switch {
...
func (d *Postgres) addColumn(c *Column) *sql.ColumnBuilder {
	...
	if c.Generated != nil && c.Generated[dialect.Postgres].Expr != "" {
		b.Attr(fmt.Sprintf("GENERATED ALWAYS AS (%s) %s", c.Generated[dialect.Postgres].Expr, c.Generated[dialect.Postgres].Type))
	} else {
		c.nullable(b)
	}
