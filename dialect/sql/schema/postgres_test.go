...
func TestPostgres_Create(t *testing.T) {
	tests := []struct {
		...
		{
			name: "create native enum",
			tables: []*Table{
				NewTable("users").
					AddPrimary(&Column{Name: "id", Type: field.TypeInt, Increment: true}).
					AddColumn(&Column{Name: "mood", Type: field.TypeEnum, SchemaType: map[string]string{dialect.Postgres: "mood"}, Enums: []string{"happy", "sad", "ok"}}),
			},
			before: func(mock pgMock) {
				mock.start("120000")
				mock.nativeEnumTypes()
				mock.createEnum("mood", "happy", "sad", "ok")
				mock.tableExists("users", false)
				mock.ExpectExec(escape(`CREATE TABLE IF NOT EXISTS "users"("id" bigint GENERATED BY DEFAULT AS IDENTITY NOT NULL, "mood" mood NOT NULL, PRIMARY KEY("id"))`)).
					WillReturnResult(sqlmock.NewResult(0, 1))
				mock.ExpectCommit()
			},
		},
		{
			name: "drop native enum",
			tables: []*Table{
				NewTable("users").
					AddPrimary(&Column{Name: "id", Type: field.TypeInt, Increment: true}),
			},
			...
			before: func(mock pgMock) {
				mock.start("120000")
				mock.nativeEnumTypes(nativeEnum{
					name:   "mood",
					labels: []string{"happy", "sad", "ok"},
				})
				mock.tableExists("users", true)
				mock.ExpectQuery(escape(`SELECT "column_name", "data_type", "is_nullable", "column_default", "udt_name" FROM INFORMATION_SCHEMA.COLUMNS WHERE "table_schema" = CURRENT_SCHEMA() AND "table_name" = $1`)).
					WithArgs("users").
					WillReturnRows(sqlmock.NewRows([]string{"column_name", "data_type", "is_nullable", "column_default", "udt_name"}).
						...
						AddRow("mood", "USER-DEFINED", "NO", "NULL", "mood"))
				mock.ExpectQuery(escape(fmt.Sprintf(indexesQuery, "users"))).
					WillReturnRows(sqlmock.NewRows([]string{"index_name", "column_name", "primary", "unique", "seq_in_index"}).
						AddRow("users_pkey", "id", "t", "t", 0))
				mock.ExpectExec(escape(`ALTER TABLE "users" DROP COLUMN "mood"`)).
					WillReturnResult(sqlmock.NewResult(0, 1))
				mock.dropEnum("mood")
				mock.ExpectCommit()
			},
		...
		{
			name: "alter string to native enum",
			tables: []*Table{
				NewTable("users").
					AddPrimary(&Column{Name: "id", Type: field.TypeInt, Increment: true}).
					AddColumn(&Column{Name: "mood", Type: field.TypeEnum, SchemaType: map[string]string{dialect.Postgres: "mood"}, Enums: []string{"happy", "sad", "ok"}}),
			...
			before: func(mock pgMock) {
				...
				mock.ExpectQuery(escape(`SELECT "column_name", "data_type", "is_nullable", "column_default", "udt_name" FROM INFORMATION_SCHEMA.COLUMNS WHERE "table_schema" = CURRENT_SCHEMA() AND "table_name" = $1`)).
					...
					WillReturnRows(sqlmock.NewRows([]string{"column_name", "data_type", "is_nullable", "column_default", "udt_name"}).
						AddRow("id", "bigint", "NO", "NULL", "NULL").
						AddRow("mood", "character varying", "NO", "NULL", "NULL"))
				mock.ExpectQuery(escape(fmt.Sprintf(indexesQuery, "users"))).
					WillReturnRows(sqlmock.NewRows([]string{"index_name", "column_name", "primary", "unique", "seq_in_index"}).
						AddRow("users_pkey", "id", "t", "t", 0))
				mock.ExpectExec(`ALTER TABLE "users" ALTER COLUMN "mood" TYPE mood using \(mood::mood\), ALTER COLUMN "mood" SET NOT NULL`).
					WillReturnResult(sqlmock.NewResult(0, 1))
				mock.ExpectCommit()
			},
		},
		{
			name: "alter native enum to string",
			tables: []*Table{
				NewTable("users").
					AddPrimary(&Column{Name: "id", Type: field.TypeInt, Increment: true}).
					AddColumn(&Column{Name: "mood", Type: field.TypeString}),
			},
			before: func(mock pgMock) {
				mock.start("120000")
				mock.nativeEnumTypes()
				...
				mock.ExpectQuery(escape(fmt.Sprintf(indexesQuery, "users"))).
					WillReturnRows(sqlmock.NewRows([]string{"index_name", "column_name", "primary", "unique", "seq_in_index"}).
						AddRow("users_pkey", "id", "t", "t", 0))
				mock.ExpectExec("ALTER TABLE \"users\" ALTER COLUMN \"mood\" TYPE varchar, ALTER COLUMN \"mood\" SET NOT NULL").
					WillReturnResult(sqlmock.NewResult(0, 1))
				mock.ExpectCommit()
			},
		},
		{
			name: "add native enum label",
			tables: []*Table{
				NewTable("users").
					AddPrimary(&Column{Name: "id", Type: field.TypeInt, Increment: true}).
					AddColumn(&Column{Name: "mood", Type: field.TypeEnum, SchemaType: map[string]string{dialect.Postgres: "mood"}, Enums: []string{"happy", "sad", "ok", "ecstatic"}}),
			},
			before: func(mock pgMock) {
				mock.start("120000")
				mock.nativeEnumTypes(nativeEnum{
				...
				mock.tableEnumColumns(tableEnumColumn{
					tableName: "users",
					colName:   "mood",
					enumName:  "mood",
				})
				mock.renameEnum("mood", "mood_entmigrate")
				mock.createEnum("mood", "happy", "sad", "ok", "ecstatic")
				mock.ExpectExec(`ALTER COLUMN "mood" TYPE mood using mood::text::mood`).
					WillReturnResult(sqlmock.NewResult(0, 1))
				mock.dropEnum("mood_entmigrate")
				mock.tableExists("users", true)
				mock.ExpectQuery(escape(`SELECT "column_name", "data_type", "is_nullable", "column_default", "udt_name" FROM INFORMATION_SCHEMA.COLUMNS WHERE "table_schema" = CURRENT_SCHEMA() AND "table_name" = $1`)).
					WithArgs("users").
					WillReturnRows(sqlmock.NewRows([]string{"column_name", "data_type", "is_nullable", "column_default", "udt_name"}).
		...
		{
			name: "remove native enum label",
			tables: []*Table{
				NewTable("users").
					AddPrimary(&Column{Name: "id", Type: field.TypeInt, Increment: true}).
					AddColumn(&Column{Name: "mood", Type: field.TypeEnum, SchemaType: map[string]string{dialect.Postgres: "mood"}, Enums: []string{"happy", "sad"}}),
			},
			before: func(mock pgMock) {
				mock.start("120000")
				mock.nativeEnumTypes(nativeEnum{
				...
				mock.renameEnum("mood", "mood_entmigrate")
				mock.createEnum("mood", "happy", "sad")
				mock.ExpectExec(`ALTER COLUMN "mood" TYPE mood using mood::text::mood`).
					WillReturnResult(sqlmock.NewResult(0, 1))
				mock.dropEnum("mood_entmigrate")
				mock.tableExists("users", true)
...
func (m pgMock) nativeEnumTypes(enums ...nativeEnum) {
	rows := sqlmock.NewRows([]string{"typname", "enumlabel"})

	for _, e := range enums {
		for _, l := range e.labels {
			rows.AddRow(e.name, l)
		}
	}

	m.ExpectQuery(escape(nativeEnumsQuery)).
		WillReturnRows(rows)
}

type tableEnumColumn struct {
	tableName string
	colName   string
	enumName  string
}

func (m pgMock) tableEnumColumns(enumColumns ...tableEnumColumn) {
	rows := sqlmock.NewRows([]string{"table_name", "column_name", "udt_name"})

	for _, e := range enumColumns {
		rows.AddRow(e.tableName, e.colName, e.enumName)
	}

	m.ExpectQuery(escape(enumColumnsQuery)).
		WillReturnRows(rows)
}

func (m pgMock) createEnum(name string, values ...string) {
	...
	m.ExpectExec(fmt.Sprintf(`CREATE TYPE %s as ENUM \(%s\)`, name, strings.Join(quotedValues, ","))).
		WillReturnResult(sqlmock.NewResult(0, 1))
}

func (m pgMock) dropEnum(name string) {
	m.ExpectExec(escape(fmt.Sprintf(`DROP TYPE %s`, name))).
		WillReturnResult(sqlmock.NewResult(0, 1))
}

func (m pgMock) renameEnum(old, new string) {
	m.ExpectExec(escape(fmt.Sprintf("ALTER TYPE %s RENAME TO  %s", old, new))).
		WillReturnResult(sqlmock.NewResult(0, 1))
}
