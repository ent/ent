...
func TestOrderByNeighborsCount(t *testing.T) {
	...
	// Cross-schema M2M: the join (edge) table lives in another schema, so its
	// FROM reference must be schema-qualified. This is what the generated
	// OrderByNeighbor* helpers do by setting step.Edge.Schema from SchemaConfig.
	t.Run("M2M/Schema", func(t *testing.T) {
		s := s.Clone()
		step := NewStep(
			From("users", "id"),
			To("groups", "id"),
		...
		step.To.Schema = "db2"
		step.Edge.Schema = "db2"
		OrderByNeighborsCount(s, step)
		query, args := s.Query()
		require.Empty(t, args)
		require.Equal(t, `SELECT "users"."name" FROM "users" LEFT JOIN (SELECT "db2"."user_groups"."user_id", COUNT(*) AS "count_groups" FROM "db2"."user_groups" GROUP BY "db2"."user_groups"."user_id") AS "t1" ON "users"."id" = "t1"."user_id" ORDER BY "t1"."count_groups" NULLS FIRST`, query)
	})
	// Zero or one.
...
func TestOrderByNeighborTerms(t *testing.T) {
	...
	// Cross-schema M2M: both the target table and the join (edge) table live in
	// another schema, so their FROM/JOIN references must be schema-qualified.
	// This mirrors what the generated ByGroups(term, ...) helper does by setting
	// step.To.Schema / step.Edge.Schema from SchemaConfig.
	t.Run("M2M/Schema", func(t *testing.T) {
		s := s.Clone()
		step := NewStep(
			From("users", "id"),
		...
		query, args := s.Query()
		require.Empty(t, args)
		require.Equal(t, `SELECT "users"."name", "t1"."total_users" FROM "users" LEFT JOIN (SELECT "user_id", SUM("db2"."group"."num_users") AS "total_users" FROM "db2"."group" JOIN "db2"."user_groups" AS "t1" ON "db2"."group"."id" = "t1"."group_id" GROUP BY "user_id") AS "t1" ON "users"."id" = "t1"."user_id" ORDER BY "t1"."total_users" NULLS FIRST`, query)
	})
	// Inverse-Through M2M (e.g. ByParents on a parents/children Through edge).
	// The inverse flag swaps the join PKs; schema qualification must still apply.
	t.Run("M2M/Inverse/Schema", func(t *testing.T) {
		s := s.Clone()
		step := NewStep(
			From("users", "id"),
			To("user", "id"),
			Edge(M2M, true, "user_children", "user_id", "child_id"),
		)
		step.To.Schema = "db1"
		step.Edge.Schema = "db1"
		OrderByNeighborTerms(s, step,
			sql.OrderBySum(
				"age",
				sql.OrderSelectAs("total_age"),
			),
		)
		query, args := s.Query()
		require.Empty(t, args)
		require.Equal(t, `SELECT "users"."name", "t1"."total_age" FROM "users" LEFT JOIN (SELECT "child_id", SUM("db1"."user"."age") AS "total_age" FROM "db1"."user" JOIN "db1"."user_children" AS "t1" ON "db1"."user"."id" = "t1"."user_id" GROUP BY "child_id") AS "t1" ON "users"."id" = "t1"."child_id" ORDER BY "t1"."total_age" NULLS FIRST`, query)
	})
}
