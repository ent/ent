...
// Real for the PetDelete builder.
func (pd *PetDelete) Real() *PetDelete {
	pd.up = nil
	return pd
}
...
func (pd *PetDelete) exec(ctx context.Context) (int, error) {
	if pd.up != nil {
		pd.up.mutation = pd.mutation
		pd.up.SetDeletedAt(time.Now())
		return pd.up.Save(ctx)
	}
...
// Real for the PetDelete builder.
func (pdo *PetDeleteOne) Real() *PetDeleteOne {
	pdo.pd.Real()
	return pdo
}
