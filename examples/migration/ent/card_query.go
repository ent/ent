...
func (q *CardQuery) QueryPayments() *PaymentQuery {
	query := (&PaymentClient{config: q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (q *CardQuery) Clone() *CardQuery {
	...
	return &CardQuery{
		...
		withPayments: q.withPayments.Clone(),
...
func (q *CardQuery) WithPayments(opts ...func(*PaymentQuery)) *CardQuery {
	query := (&PaymentClient{config: q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	q.withPayments = query
	return q
}
...
func (q *CardQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Card, error) {
	...
	if query := q.withPayments; query != nil {
		if err := q.loadPayments(ctx, query, nodes,
...
func (q *CardQuery) loadPayments(ctx context.Context, query *PaymentQuery, nodes []*Card, init func(*Card), assign func(*Card, *Payment)) error {
