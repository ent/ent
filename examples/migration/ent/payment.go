...
func (m *Payment) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case payment.FieldCardID:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				m.CardID = int(value.Int64)
			}
		case payment.FieldAmount:
			if value, ok := values[i].(*sql.NullFloat64); !ok {
				return fmt.Errorf("unexpected type %T for field amount", values[i])
			} else if value.Valid {
				m.Amount = value.Float64
			}
		case payment.FieldCurrency:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field currency", values[i])
			} else if value.Valid {
				m.Currency = payment.Currency(value.String)
			}
		case payment.FieldTime:
		...
		case payment.FieldStatus:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				m.Status = payment.Status(value.String)
			}
		default:
...
func (m *Payment) Value(name string) (ent.Value, error) {
	return m.selectValues.Get(name)
}

// QueryCard queries the "card" edge of the Payment entity.
func (m *Payment) QueryCard() *CardQuery {
	return NewPaymentClient(m.config).QueryCard(m)
}
...
func (m *Payment) Update() *PaymentUpdateOne {
	return NewPaymentClient(m.config).UpdateOne(m)
}

// Unwrap unwraps the Payment entity that was returned from a transaction after it was closed,
// so that all future queries will be executed through the driver which created the transaction.
func (m *Payment) Unwrap() *Payment {
...
func (m *Payment) String() string {
	...
	builder.WriteString(fmt.Sprintf("%v", m.CardID))
	builder.WriteString(", ")
	builder.WriteString("amount=")
	builder.WriteString(fmt.Sprintf("%v", m.Amount))
	builder.WriteString(", ")
	builder.WriteString("currency=")
	builder.WriteString(fmt.Sprintf("%v", m.Currency))
	builder.WriteString(", ")
