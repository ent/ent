...
func (q *UserQuery) QueryManage() *GroupQuery {
	query := (&GroupClient{config: q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (q *UserQuery) WithManage(opts ...func(*GroupQuery)) *UserQuery {
	query := (&GroupClient{config: q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := q.withManage; query != nil {
		if err := q.loadManage(ctx, query, nodes,
...
func (q *UserQuery) loadManage(ctx context.Context, query *GroupQuery, nodes []*User, init func(*User), assign func(*User, *Group)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
