...
func (q *GroupQuery) QueryAdmin() *UserQuery {
	query := (&UserClient{config: q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (q *GroupQuery) WithAdmin(opts ...func(*UserQuery)) *GroupQuery {
	query := (&UserClient{config: q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := q.withAdmin; query != nil {
		if err := q.loadAdmin(ctx, query, nodes, nil,
...
func (q *GroupQuery) loadAdmin(ctx context.Context, query *UserQuery, nodes []*Group, init func(*Group), assign func(*Group, *User)) error {
