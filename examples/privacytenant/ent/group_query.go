...
func (q *GroupQuery) QueryTenant() *TenantQuery {
	query := (&TenantClient{config: q.config}).Query()
...
func (q *GroupQuery) WithTenant(opts ...func(*TenantQuery)) *GroupQuery {
	query := (&TenantClient{config: q.config}).Query()
...
func (q *GroupQuery) prepareQuery(ctx context.Context) error {
	...
	if err := group.Policy.EvalQuery(ctx, q); err != nil {
		return err
	}
	return nil
}
...
func (q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := q.withTenant; query != nil {
		if err := q.loadTenant(ctx, query, nodes, nil,
...
func (q *GroupQuery) loadTenant(ctx context.Context, query *TenantQuery, nodes []*Group, init func(*Group), assign func(*Group, *Tenant)) error {
