...
func (c *GroupClient) QueryTenant(m *Group) *TenantQuery {
...
// UpdateOne returns an update builder for the given entity.
func (c *TenantClient) UpdateOne(m *Tenant) *TenantUpdateOne {
	mutation := newTenantMutation(c.config, OpUpdateOne, withTenant(m))
...
// DeleteOne returns a builder for deleting the given entity.
func (c *TenantClient) DeleteOne(m *Tenant) *TenantDeleteOne {
	return c.DeleteOneID(m.ID)
}
...
func (c *UserClient) QueryTenant(m *User) *TenantQuery {
	query := (&TenantClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := m.ID
		step := sqlgraph.NewStep(
