...
type (
	...
	Query interface{}
	// Mutation represents an operation that mutates the graph.
	...
	Mutation interface {
		...
		// FieldDefault returns the default value of a field with
		// the given name. The second boolean return value indicates
		// that the field has a default defined in the schema.
		FieldDefault(name string) (Value, bool)
