...
In this column, users may store JSON objects of arbitrary schema:
...
be stored in regular VARCHAR or TEXT columns, but when a column is declared
...
func TestEntJSON(t *testing.T) {
	...
	// Count how many users have more than one comment.
	count := client.User.Query().
		Where(func(s *sql.Selector) {
			s.Where(
				sqljson.LenGT(user.FieldComments, 0, sqljson.DotPath(".")),
			)
		}).
		CountX(ctx)
...
Preferably, appends are implemented in a way that is _atomic_, that is, without
...
In a similar way, to avoid the need to synchronize writes using optimistic locking
or some other mechanism, let's see how we can leverage the database's capability to
perform them concurrently in a safe manner.

Two things to note as we are constructing this query. First, the syntax for working
...
func TestAppendSubfield(t *testing.T) {
	...
	u.Update().
		Modify(func(u *sql.UpdateBuilder) {
			sqljson.Append(u, user.FieldMeta, []string{"goodbye"}, sqljson.DotPath(".tags"))
		}).
		ExecX(ctx)
	// highlight-end
...
in popular SQL databases. Finally, we showed how to use do this using Ent.
