...
If you are using the Atlas migration engine you are able to use the versioned migrations feature of it. In order to have
Ent make the necessary changes to your code, you have to enable this feature with one of the two options:
...
func main() {
    ...
    ctx := context.Background()
    // Run migration.
    err = client.Schema.Diff(ctx, schema.WithDir("migrations"))
    if err != nil {
        log.Fatalf("failed creating schema resources: %v", err)
    }
}
