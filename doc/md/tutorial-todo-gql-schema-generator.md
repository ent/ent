...
The `entgql.RelayConnection()` annotation is used to generate the Relay Edge, Connection, and PageInfo types for the Todo type. The `entgql.QueryField()` annotation is used to generate the `todos` field in the `Query` type.

```go {15,16} title="ent/schema/todo.go"
// Edges of the Todo.
...
Please remove the bellow types from the `todo.graphql` to avoid conflict with the types are generated by EntGQL in the `ent.graphql` file.
...
First, please remove the `ent/generate.go` file. Then, update the `ent/entc.go` file with the correct path to files, because the Ent codegen will be run from the project root directory.
...
You will see the `ent.graphql` file will be updated with the new content from the EntGQL's Schema Generator.
...
You also do the extend for any type that generated by Ent. To skip a field from the type, you can use the `entgql.Skip()` on that field or edge.

---

Well done! As you can see, after adapted the Schema Generator feature we don't have to write GQL schema by hand anymore. Have questions? Need help with getting started? Feel free to join our [Slack channel](https://entgo.io/docs/slack).
