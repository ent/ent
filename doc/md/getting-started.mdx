...
Ent provides two approaches for running schema migrations: [Automatic Migration](/docs/migrate) and
[Versioned migration](/docs/versioned-migrations). Here is a brief overview of each approach:

### Automatic Migration

With Automatic Migration, users can use the following API to keep the database schema aligned with the schema objects
...
This approach is mostly useful for prototyping, development, or testing. However, it is recommended to use the
_Versioned Migration_ approach for mission-critical production environments. Using versioned migrations users know
beforehand what changes are being applied to their database, and can easily tune them for their needs.

Read more about this approach in the [Automatic Migration](/docs/migrate) documentation.

### Versioned Migration

Unlike _Automatic Migration_, the _Version Migrations_ approach uses Atlas to automatically generate a set of migration
