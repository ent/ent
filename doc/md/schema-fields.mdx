...
In addition to `github.com/google/uuid`, `field.UUID` accepts Go 1.27's standard `uuid.UUID` type
(and its pointer variant), even though it does not implement `driver.Valuer` or `sql.Scanner`:
