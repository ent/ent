...
func (q *GroupQuery) QueryTags() *TagQuery {
...
func (q *GroupQuery) QueryJoinedUsers() *UserGroupQuery {
...
func (q *GroupQuery) QueryGroupTags() *GroupTagQuery {
...
func (q *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withJoinedUsers: q.withJoinedUsers.Clone(),
...
func (q *GroupQuery) WithTags(opts ...func(*TagQuery)) *GroupQuery {
	query := (&TagClient{config: q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (q *GroupQuery) WithJoinedUsers(opts ...func(*UserGroupQuery)) *GroupQuery {
	query := (&UserGroupClient{config: q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	q.withJoinedUsers = query
	return q
}
...
func (q *GroupQuery) WithGroupTags(opts ...func(*GroupTagQuery)) *GroupQuery {
	query := (&GroupTagClient{config: q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := q.withJoinedUsers; query != nil {
		if err := q.loadJoinedUsers(ctx, query, nodes,
...
func (q *GroupQuery) loadTags(ctx context.Context, query *TagQuery, nodes []*Group, init func(*Group), assign func(*Group, *Tag)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Group)
	nids := make(map[int]map[*Group]struct{})
	for i, node := range nodes {
...
func (q *GroupQuery) loadJoinedUsers(ctx context.Context, query *UserGroupQuery, nodes []*Group, init func(*Group), assign func(*Group, *UserGroup)) error {
...
func (q *GroupQuery) loadGroupTags(ctx context.Context, query *GroupTagQuery, nodes []*Group, init func(*Group), assign func(*Group, *GroupTag)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
