...
// HasFavouriteFood applies the HasEdge predicate on the "favourite_food" edge.
func HasFavouriteFood() predicate.Pet {
	return predicate.Pet(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(FavouriteFoodTable, FieldID),
			sqlgraph.Edge(sqlgraph.M2O, false, FavouriteFoodTable, FavouriteFoodColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasFavouriteFoodWith applies the HasEdge predicate on the "favourite_food" edge with a given conditions (other predicates).
func HasFavouriteFoodWith(preds ...predicate.Food) predicate.Pet {
	return predicate.Pet(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(FavouriteFoodInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.M2O, false, FavouriteFoodTable, FavouriteFoodColumn),
		)
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
