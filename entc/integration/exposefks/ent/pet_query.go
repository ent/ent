...
// QueryFavouriteFood chains the current query on the favourite_food edge.
func (pq *PetQuery) QueryFavouriteFood() *FoodQuery {
	query := &FoodQuery{config: pq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, false, pet.FavouriteFoodTable, pet.FavouriteFoodColumn),
		)
		fromU = sqlgraph.SetNeighbors(pq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (pq *PetQuery) Clone() *PetQuery {
	...
	return &PetQuery{
		...
		withFavouriteFood: pq.withFavouriteFood.Clone(),
...
//  WithFavouriteFood tells the query-builder to eager-loads the nodes that are connected to
// the "favourite_food" edge. The optional arguments used to configure the query builder of the edge.
func (pq *PetQuery) WithFavouriteFood(opts ...func(*FoodQuery)) *PetQuery {
	query := &FoodQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withFavouriteFood = query
	return pq
}
...
func (pq *PetQuery) sqlAll(ctx context.Context) ([]*Pet, error) {
	...
	if query := pq.withOwner; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "UserPets" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := pq.withFavouriteFood; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Pet)
		for i := range nodes {
			if fk := nodes[i].PetFavouriteFood; fk != nil {
				ids = append(ids, *fk)
				nodeids[*fk] = append(nodeids[*fk], nodes[i])
			}
		}
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "PetFavouriteFood" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.FavouriteFood = n
			}
		}
	}
