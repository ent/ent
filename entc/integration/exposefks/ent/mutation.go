...
// newFoodMutation creates new mutation for Food.
...
func (m *PetMutation) SetFavouriteFoodID(id int) {
	m.favourite_food = &id
}

// ClearFavouriteFood clears the favourite_food edge to Food.
func (m *PetMutation) ClearFavouriteFood() {
	m.clearedfavourite_food = true
}

// FavouriteFoodCleared returns if the edge favourite_food was cleared.
func (m *PetMutation) FavouriteFoodCleared() bool {
	return m.clearedfavourite_food
}

// FavouriteFoodID returns the favourite_food id in the mutation.
func (m *PetMutation) FavouriteFoodID() (id int, exists bool) {
	if m.favourite_food != nil {
		return *m.favourite_food, true
	}
	return
}

// FavouriteFoodIDs returns the favourite_food ids in the mutation.
// Note that ids always returns len(ids) <= 1 for unique edges, and you should use
// FavouriteFoodID instead. It exists only for internal usage by the builders.
func (m *PetMutation) FavouriteFoodIDs() (ids []int) {
	if id := m.favourite_food; id != nil {
		ids = append(ids, *id)
	}
	return
}

// ResetFavouriteFood reset all changes of the "favourite_food" edge.
func (m *PetMutation) ResetFavouriteFood() {
	m.favourite_food = nil
	m.clearedfavourite_food = false
}
...
func (m *PetMutation) AddedEdges() []string {
	...
	if m.favourite_food != nil {
		edges = append(edges, pet.EdgeFavouriteFood)
	}
	return edges
}
