	...
	for i := range columns {
		...
		case task.FieldStatus:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				m.Status = task.Status(value.String)
			}
		case task.FieldUUID:
		...
		case task.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				...
				m.user_tasks = new(int)
				*m.user_tasks = int(value.Int64)
			}
		default:
...
func (m *Task) QueryTeams() *TeamQuery {
	return NewTaskClient(m.config).QueryTeams(m)
}

// QueryOwner queries the "owner" edge of the Task entity.
func (m *Task) QueryOwner() *UserQuery {
	return NewTaskClient(m.config).QueryOwner(m)
}
