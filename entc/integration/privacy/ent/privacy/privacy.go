...
// The PlanetReadRuleFunc type is an adapter to allow the use of ordinary
// functions as a read rule.
type PlanetReadRuleFunc func(context.Context, *ent.Planet) error

// EvalRead calls f(ctx, v).
func (f PlanetReadRuleFunc) EvalRead(ctx context.Context, v ent.Value) error {
	if v, ok := v.(*ent.Planet); ok {
		return f(ctx, v)
	}
	return Denyf("ent/privacy: unexpected value type %T, expect *ent.Planet", v)
}

// The PlanetWriteRuleFunc type is an adapter to allow the use of ordinary
// functions as a write rule.
type PlanetWriteRuleFunc func(context.Context, *ent.PlanetMutation) error

// EvalWrite calls f(ctx, m).
func (f PlanetWriteRuleFunc) EvalWrite(ctx context.Context, m ent.Mutation) error {
