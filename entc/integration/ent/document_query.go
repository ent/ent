...
func (_q *DocumentQuery) loadBlobFields(ctx context.Context, nodes []*Document) error {
	_blobs := ent.NewBlobStore(_q.blobOpeners.Document)
	for _, n := range nodes {
		if n.attachment_key != nil && *n.attachment_key != "" {
			switch data, err := _blobs.Read(ctx, document.FieldAttachment, *n.attachment_key); {
			case err != nil:
				return errors.Join(fmt.Errorf("loading attachment: %w", err), _blobs.Close())
			case data != nil:
				n.Attachment = data
			}
		}
		if n.metadata_key != nil && *n.metadata_key != "" {
			...
			if data == nil {
				return errors.Join(fmt.Errorf("loading metadata: object %q not found in blob storage", *n.metadata_key), _blobs.Close())
			}
			n.Metadata = data
		}
		if n.payload_key != nil && *n.payload_key != "" {
			switch data, err := _blobs.Read(ctx, document.FieldPayload, *n.payload_key); {
			case err != nil:
				return errors.Join(fmt.Errorf("loading payload: %w", err), _blobs.Close())
			case data != nil:
				sv := document.ValueScanner.Payload.ScanValue()
				if err := sv.Scan(data); err != nil {
					return errors.Join(fmt.Errorf("scanning payload: %w", err), _blobs.Close())
				}
				v, err := document.ValueScanner.Payload.FromValue(sv)
				if err != nil {
					return errors.Join(fmt.Errorf("scanning payload: %w", err), _blobs.Close())
				}
				n.Payload = v
		...
		if n.description_key != nil && *n.description_key != "" {
			data, err := _blobs.Read(ctx, document.FieldDescription, *n.description_key)
			if err != nil {
				return errors.Join(fmt.Errorf("loading description: %w", err), _blobs.Close())
			}
			if data == nil {
				return errors.Join(fmt.Errorf("loading description: object %q not found in blob storage", *n.description_key), _blobs.Close())
			}
			n.Description = string(data)
		}
	}
	return _blobs.Close()
}
