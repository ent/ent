...
func (gq *GroupQuery) sqlAll(ctx context.Context) ([]*Group, error) {
	...
	if query := gq.withFiles; query != nil {
		...
		for _, n := range neighbors {
			fk := n.file_group_id
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "file_group_id" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "file_group_id" returned %v for node %v`, *fk, n.ID)
			}
	...
	if query := gq.withBlocked; query != nil {
		...
		for _, n := range neighbors {
			fk := n.blocked_group_id
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "blocked_group_id" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "blocked_group_id" returned %v for node %v`, *fk, n.ID)
			}
