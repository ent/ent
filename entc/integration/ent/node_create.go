...
func (c *NodeCreate) createSpec() (*Node, *sqlgraph.CreateSpec) {
	...
	if nodes := c.mutation.PrevIDs(); len(nodes) > 0 {
	...
	if nodes := c.mutation.NextIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2O,
			Inverse: false,
...
//		}).
//		Exec(ctx)
func (c *NodeCreate) OnConflict(opts ...sql.ConflictOption) *NodeUpsertOne {
	c.conflict = opts
	return &NodeUpsertOne{create: c}
}
...
//		Exec(ctx)
func (c *NodeCreate) OnConflictColumns(columns ...string) *NodeUpsertOne {
...
//		}).
//		Exec(ctx)
func (c *NodeCreateBulk) OnConflict(opts ...sql.ConflictOption) *NodeUpsertBulk {
	c.conflict = opts
	return &NodeUpsertBulk{create: c}
}
...
//		OnConflict(sql.ConflictColumns(columns...)).
//		Exec(ctx)
func (c *NodeCreateBulk) OnConflictColumns(columns ...string) *NodeUpsertBulk {
	c.conflict = append(c.conflict, sql.ConflictColumns(columns...))
	return &NodeUpsertBulk{create: c}
}
