...
func TestPostgreSQL(t *testing.T) {
	for version, port := range map[string]int{"11": 5432} {
		t.Run(version, func(t *testing.T) {
			client, err := ent.Open("postgres", fmt.Sprintf("postgres://postgres:pass@localhost:%d/test?sslmode=disable", port))
			require.NoError(t, err)
			defer client.Close()
			require.NoError(t, client.Schema.Create(context.Background()))
			for _, tt := range tests {
...
func UniqueConstraint(t *testing.T, client *ent.Client) {
	...
	require.True(ent.IsConstraintFailure(err), "c1 already has a parent")
	_, err = client.User.Create().SetAge(10).SetName("z").SetNickname("z").AddChildren(c2).Save(ctx)
	require.True(ent.IsConstraintFailure(err), "c2 already has a parent")
